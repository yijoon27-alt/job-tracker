...
- 2026-09-07 3차 작업으로 서류 마감 시각, AI 역량검사/인적성 마감일시와 응시 완료 체크, 작성 완료 공고 하단 정렬을 추가했다. 이 파일도 아직 GitHub에 업로드해야 한다.
...
- 3차 작업분은 `deadline_time`, `assessment_deadline`, `assessment_time`, `assessment_done` 필드를 전제로 한다.
...
- 역량검사는 전형 결과가 아니라 마감 관리 항목이므로 서류·1차·2차·최종 단계 흐름에는 넣지 않는다.
- 정렬은 `종료 여부 → 할 일 남음 → 마감 지남 → 다음 마감일` 순으로 판단한다.
...
- 마감 칸의 `D-Day` 배지와 날짜는 정렬 기준과 무관하게 언제나 실제 서류 마감을 그대로 보여준다.
...
- CSS와 JavaScript에는 `20260907-time-text` 캐시 버전이 적용되어 있다.
...
- AI 역량검사/인적성: `assessment_deadline date`, `assessment_time time`, `assessment_done boolean not null default false`
...
`deadline_time`, `assessment_deadline`, `assessment_time`은 모두 null 허용이고 `assessment_done`은 기본값이 `false`라, 기존 기록은 값 보정 없이 그대로 유지된다.
