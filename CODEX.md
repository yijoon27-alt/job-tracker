...
- 정렬은 `종료 여부 → 할 일 남음 → 진행 중 여부 → 마감 지남 → 다음 마감일` 순으로 판단한다. 실제로 해야 할 마감이 남은 공고가 먼저이고, 그다음에는 서류 합격 후 역검·면접을 진행 중인 공고를 모은다.
...
- 요약 카드는 클릭 가능한 빠른 필터다. 전체·서류 작성·서류 결과 대기·서류 합격·진행 중 공고를 선택할 수 있고, 같은 카드를 다시 누르면 전체 보기로 돌아간다. 카드 선택 시 검색어와 두 드롭다운 필터를 초기화하며, 드롭다운 필터를 직접 바꾸면 카드 선택 표시를 해제한다.
...
- CSS와 JavaScript에는 `20260911-clickable-stages` 캐시 버전이 적용되어 있다.
